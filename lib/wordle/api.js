...
// anonymous players — this is account-tied tracking only.
export async function recordAttempt(won, tries) {
  const { error } = await supabase.rpc("wordle_record_attempt", { p_won: won, p_tries: tries });
  if (error) console.error("recordAttempt failed", error);
}