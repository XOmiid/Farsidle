...
export async function fetchTodayReveal() {
  const { data, error } = await supabase.rpc("duel_get_today_reveal");
  if (error) {
    console.error("duel fetchTodayReveal failed", error);
    return null;
  }
  return data || []; // [{ prompt_a, unit_a, value_a, prompt_b, unit_b, value_b }, ...]
}
...
export async function finalize() {
  ...
  return { data: data[0].score, streak: data[0].streak || 0, error: null };
}
