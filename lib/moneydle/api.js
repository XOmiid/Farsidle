...
export async function fetchTodayReveal() {
  const { data, error } = await supabase.rpc("moneydle_get_today_reveal");
  if (error || !data || !data.length) {
    console.error("moneydle fetchTodayReveal failed", error);
    return null;
  }
  return data[0].correct_order; // [{ code, name_fa, usd_value }, ...] or null
}
