...
const CHORD_FREQUENCIES = {
  ...
  // Returns { promise, cancel } — call cancel() to stop mid-sequence.
  export function playSequence(chordIndices) {
    const GAP = 0.3;
    const DURATION = 1.0;
    const timers = [];
    let cancelled = false;
    ...
    const promise = new Promise((resolve) => {
      chordIndices.forEach((idx, i) => {
        const delay = i * (DURATION + GAP) * 1000;
        const t = setTimeout(() => {
          if (!cancelled) playChord(idx, DURATION);
        }, delay);
        timers.push(t);
      });
