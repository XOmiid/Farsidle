...
export default function ChordleGame() {
  ...
  useEffect(() => {
    if (!howtoOpen) {
      try { localStorage.setItem("fa-chordle-v2-howto-seen", "1"); } catch {}
    }
  }, [howtoOpen]);
  ...
  const handleContinue = useCallback(() => {
    if (!reveal) return;
    if (reveal.game_over) { setGameOver(true); openResult(); return; }
    initRound(round + 1);
        ...
        <>
          ...
          <div className="w-full max-w-[420px] mb-4">
            <p className="text-[.75rem] text-ivory-dim mb-2 text-right">
              {selected !== null ? "روی یه خانه بزن" : "یه دکمه انتخاب کن"}
            </p>
            <div className="flex gap-2 justify-center">
              {slots.map((val, i) => {
                const color = val !== null ? CHORD_COLORS[val - 1] : undefined;
                const revealColor = reveal ? (reveal.correct_slots[i] ? "#4ade80" : "#ef4444") : color;
                return (
                  <button key={i} onClick={() => handleSlotClick(i)}
                    ...
                    style={{
                      borderColor: revealColor ?? (selected !== null ? "#4ade80" : "#2a3d2e"),
                    ...
                    {val !== null ? toPersianDigits(val) : (
                      <span className="text-[.8rem] opacity-30">{toPersianDigits(i + 1)}</span>
                    )}
                  </button>
                );
              })}
            ...
            <div className="w-full max-w-[420px] mb-4">
              ...
              <div className="grid grid-cols-3 gap-2.5">
                {Array.from({ length: 9 }, (_, i) => i + 1).map((idx) => {
                  ...
                  const color = instrument === "drums" ? DRUM_KIT[idx]?.color : CHORD_COLORS[idx - 1];
                  const label = instrument === "drums" ? DRUM_KIT[idx]?.name : toPersianDigits(idx);
                  return (
            ...
            <div className="w-full max-w-[420px] rounded-xl px-4 py-3 text-center mb-3 border border-green-dim bg-white/[.03]">
              ...
              <div className="flex gap-1.5 justify-center mt-1">
                {reveal.correct_slots.map((ok, i) => (
                  <span key={i} className={ok ? "text-green" : "text-red-400"}>{ok ? "✓" : "✗"}</span>
                ))}
              </div>
            </div>
          )}
          ...
          {/* Submit / Continue */}
          {phase === "pick" && !reveal ? (
            <button onClick={handleSubmit} disabled={submitting || slots.some((s) => s === null)}
