...
export default function ChordleGame() {
  ...
  const [howtoOpen, setHowtoOpen] = useState(() => {
    try {
      return !localStorage.getItem("fa-chordle-v2-howto-seen");
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    if (!howtoOpen) {
      try {
        localStorage.setItem("fa-chordle-v2-howto-seen", "1");
      } catch {}
    }
  }, [howtoOpen]);
  ...
  useEffect(() => {
    ...
    (async () => {
      const [puz, status] = await Promise.all([
        fetchTodayPuzzle(),
        checkTodayStatus(),
      ]);
  ...
  const handleSubmit = useCallback(async () => {
    if (slots.some((s) => s === null)) {
      showToast("همه خانه‌ها باید پر باشن");
      return;
    }
    ...
    if (data.game_over) {
      setFinalScore(totalScore + data.round_score);
    }
        ...
        <>
          ...
          <div className="w-full max-w-[420px] flex items-center justify-between mb-4 px-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-ivory text-[.9rem] font-bold">
                {roundInfo?.label}
              </span>

              <span className="text-ivory-dim text-[.78rem]">
                دور {toPersianDigits(round)} از ۳
              </span>
            </div>

            <div className="flex items-center gap-2">
              <span className="text-green font-bold text-[.85rem]">
                {toPersianDigits(totalScore)}/۱۵
              </span>
                ...
                return (
                  ...
                  >
                    {val !== null ? (
                      ...
                      <span className="text-[.8rem] opacity-30">
                        {toPersianDigits(i + 1)}
                      </span>
                    )}
                  </button>
                  ...
                  (idx) => {
                    ...
                    return (
                      ...
                      >
                        {instrument === "drums" ? (
                          <>
                            <span className="text-[.6rem] font-semibold leading-tight text-center px-1">
                              {label}
                            </span>

                            <span className="text-[.55rem] opacity-70">
                              {toPersianDigits(idx)}
                            </span>
                          </>
                        ) : (
          ...
          {reveal && (
            <div className="w-full max-w-[420px] rounded-xl px-4 py-3 text-center mb-3 border border-green-dim bg-white/[.03]">
              <p className="font-bold text-[.95rem] mb-1 text-ivory">
                ...
                {reveal.round_score === 5
                  ? " 🎯"
                  : reveal.round_score === 0
                  ? " 😅"
                  : " 👍"}
              </p>
          ...
          {phase === "pick" && !reveal ? (
            <button
              ...
              disabled={
                submitting || slots.some((s) => s === null)
              }
      ...
      {gameOver && !resultOpen && (
        <div className="flex flex-col items-center gap-3 mt-4">
          <p className="text-green font-bold text-xl">
            {toPersianDigits(finalScore ?? 0)}/۱۵
          </p>
