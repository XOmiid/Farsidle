    ...
    <div className="fixed inset-0 bg-[rgba(2,8,3,.86)] flex items-center justify-center z-30 p-3">
      <div className="relative bg-bg-1 border border-green-dim rounded-2xl max-w-[420px] w-full text-center max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <h2 className="font-display text-[1.6rem] m-0 mb-1">
          {total === 10 ? "حافظه کامل!" : "نتیجه‌ی امروز"}
        </h2>
        <div className="text-[3.5rem] font-extrabold text-green leading-none my-2">
          {toPersianDigits(total)}
          <span className="text-xl text-ivory-dim">/۱۰</span>
        </div>

        {/* Per-level breakdown */}
        <div className="flex gap-3 justify-center mb-3">
          <div className="bg-white/[.04] border border-border rounded-lg px-4 py-2 text-center">
            <p className="text-ivory-dim text-[.72rem] mb-0.5">مرحله ۱</p>
            <p className="text-green font-bold text-[1rem]">{toPersianDigits(l1)}/۵</p>
          </div>
          <div className="bg-white/[.04] border border-border rounded-lg px-4 py-2 text-center">
            <p className="text-ivory-dim text-[.72rem] mb-0.5">مرحله ۲</p>
            <p className="text-green font-bold text-[1rem]">{toPersianDigits(l2)}/۵</p>
          </div>
        </div>
