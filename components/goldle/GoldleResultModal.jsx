...
export default function GoldleResultModal({
  open,
  finalGold,
  questionsReached,
  streak,
  ...
  const survived = questionsReached === 5 && finalGold > 0;
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(2,8,3,.86)] flex items-center justify-center z-30 p-5 overflow-y-auto">
      <div className="relative bg-bg-1 border border-green-dim rounded-2xl p-[26px_24px] max-w-[420px] w-full text-center">
        ...
        <h2 className="font-display text-[1.7rem] m-0 mb-1">
          {finalGold === 0 ? "طلات تموم شد 😅" : survived ? "تا آخر رسیدی! 🎉" : "نتیجه‌ی امروز"}
        </h2>
        <div className="text-[3rem] font-extrabold text-green leading-none my-2">
          {finalGold !== null ? toPersianDigits(finalGold) : "-"}
          <span className="text-lg text-ivory-dim"> طلا</span>
        </div>
        {questionsReached !== null && (
          <p className="text-ivory-dim text-[.8rem] mb-1">
            تا سوال {toPersianDigits(questionsReached)} از ۵ پیش رفتی
          </p>
        )}
        {streak >= 2 && (
          <p className="mb-2">
        ...
        <div className="my-3.5 text-right">
          ...
          {user && !alreadySubmitted && finalGold !== null && (
